...
public class HelloArActivity extends AppCompatActivity implements SampleRender.Renderer {
    ...
    public HelloArActivity() throws ReaderException {
    }
...
//                createSession();
    ...
    private void performHitTest(ReaderResult result) {
        // Get location coordinates
        Object obj =
                result.getMetadata().getValue(DataDictionary.BarcodeLocation);
        ...
        frame.transformCoordinates2d(Coordinates2d.IMAGE_PIXELS, inputVertices,
                Coordinates2d.VIEW, outputVertices);

        // Test center point of coordinates for intersection on plane
        float[] midPoint = new float[0];
    ...
    CameraHelper cameraHelper = CameraHelper.Builder().build();
    ...
    private void onUpdate(FrameTime frameTime) {
        ...
        try
        {
        // Pass ImageData to DMSDK
            VideoCaptureReader videoCaptureReader = null;
    ...
    private ImageData convertImage(Image image) {
        ...
        boolean planeBuffers = false;
        for (int i = 0; i < 3; i++ )
        {
            if ( planeBuffers[i] == null || planeBuffers[i].capacity() <
                    planes[i].getBuffer().capacity() )
            {
                planeBuffers[i] = ByteBuffer.allocateDirect(
                        planes[i].getBuffer().capacity() );
            }
            planeBuffers[i].clear();
            planeBuffers[i].put( planes[i].getBuffer() );
        }
...
//    VideoCaptureReader videoCaptureReader = new VideoCaptureReader(BaseReader.All_Image_Readers,
//            null,
//            CaptureFormat.YUV420P)
//    {
//
//        @Override
//        protected void notifyOfReadResults( ReaderResult result, ResultType resultType) {
//            super.notifyOfReadResults(result, resultType);
//            performHitTest(result);
//        }
//
//        @Override
//        protected void notifyOfError(ReaderError readerError, ResultType resultType) {
//            super.notifyOfError(readerError, resultType);
//            //Handle Error
//            Toast.makeText(HelloArActivity.this, "Video Capture Reader Error", Toast.LENGTH_SHORT).show();
//        }
//
//
//    };
...
//                false );
    ...
    private void configureSession() {
        ...
        // ini  yg kode dibawah ini beda cara manggilnya sama yang di web developer.google..
        //yg di developer.google Config config = new Config(session); seperti method yg bawah
        List<CameraConfig> cameraConfigs = session.getSupportedCameraConfigs();

        // Pick the largest one
        CameraConfig cameraConfig = cameraConfigs.stream().max(Comparator.comparing(o
                -> o.getImageSize().getWidth())).get();
...
//        session.configure(config);
//
//        //kalau configure config diatas error, uncomment yg dibawah ini ndro:
//        //configureSession();
//    }
